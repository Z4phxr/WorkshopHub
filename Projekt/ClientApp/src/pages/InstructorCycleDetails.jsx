...
export default function InstructorCycleDetails(){
  const { id } = useParams();
  ...
  if (!data || !data.cycle) return <div style={{ minHeight:'100vh', padding:24 }}><div style={{ maxWidth:800, margin:'0 auto' }}><div style={{ padding:12, background:'#fff7ed', border:'1px solid #fcd34d', color:'#92400e', borderRadius:8 }}>Cycle data is not available.</div></div></div>;
    ...
    <div style={{ minHeight:'100vh', background:'linear-gradient(to bottom,#f9fafb,#ffffff)' }}>
      ...
      <div style={{ maxWidth:1200, margin:'32px auto', padding:'0 24px 80px 24px' }}>
        ...
        <div style={{ background:'white', padding:20, borderRadius:12, boxShadow:'0 2px 8px rgba(0,0,0,0.05)' }}>
          ...
          {/* disable clicking to user details in instructor view */}
          <EnrollmentList enrollments={enrollments} showHeader={false} disableRowClick={true} />
        </div>
      </div>
    </div>
  );
